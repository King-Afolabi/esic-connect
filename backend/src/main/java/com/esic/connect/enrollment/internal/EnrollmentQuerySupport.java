 ...
 * — toute autre valeur lève {@link EnrollmentException.Kind#INVALID_SORT}
 ...
 * {@code academic.internal.AcademicQuerySupport}.
 */
final class EnrollmentQuerySupport {

    static final int MAX_PAGE_SIZE = 100;
    static final int DEFAULT_PAGE_SIZE = 20;
    static final int MAX_QUERY_LENGTH = 100;

    private EnrollmentQuerySupport() {
    }
    ...
    private static Sort parseSort(String sort, Set<String> sortableFields, Sort defaultSort) {
        ...
        if (parts.length == 2) {
            direction = Sort.Direction.fromOptionalString(parts[1].trim())
                    .orElseThrow(() -> new EnrollmentException(EnrollmentException.Kind.INVALID_SORT));
        }
