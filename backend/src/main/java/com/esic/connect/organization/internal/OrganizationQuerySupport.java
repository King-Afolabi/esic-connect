 ...
 * — toute autre valeur lève {@link OrganizationException.Kind#INVALID_SORT}
 * plutôt que d'être réinterprétée silencieusement. Normalise aussi le
 * filtre texte (trim, minuscules, longueur bornée).
 */
final class OrganizationQuerySupport {

    static final int MAX_PAGE_SIZE = 100;
    static final int DEFAULT_PAGE_SIZE = 20;
    static final int MAX_QUERY_LENGTH = 100;

    private OrganizationQuerySupport() {
    }
    ...
    private static Sort parseSort(String sort, Set<String> sortableFields, Sort defaultSort) {
        ...
        if (!sortableFields.contains(field)) {
            throw new OrganizationException(OrganizationException.Kind.INVALID_SORT);
        }
        Sort.Direction direction = Sort.Direction.ASC;
        if (parts.length == 2) {
            direction = Sort.Direction.fromOptionalString(parts[1].trim())
                    .orElseThrow(() -> new OrganizationException(OrganizationException.Kind.INVALID_SORT));
        }
