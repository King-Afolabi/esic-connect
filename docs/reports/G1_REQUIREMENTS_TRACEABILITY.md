...
| G1-C | Cycle de vie avancé des séances | EF-SES-004, EF-SES-005, RG-012, RG-015, RG-017 (CDC §43) | DEC-G1-004, DEC-G1-005 | `NOT_IMPLEMENTED` (séances exceptionnelles `PLANNED→OPEN→CLOSED` seulement) |
...
| EF-SES-001 | CDC §44 (`EF-SES-001 \| Créer des séances depuis le planning \| MUST`) ; CDC §43 RG-016 | Créer des séances depuis le planning | `HORS_PÉRIMÈTRE_ASSUMÉ` (CDC §4.5.1) | La publication crée / réutilise / supersède des `course_session` via un **port public** de `coursesession` (aucune entité JPA partagée) ; `exceptional = false` ; lien `planning_entry ↔ session_public_id` | `coursesession.PlanningSessionWriter` + `DefaultPlanningSessionWriter` | test création + réutilisation + supersession ; `ModularityTests` vert | Séance issue du planning dans `/sessions` |
...
|---|---|---|---|
| RG-016 | CDC §43 (`RG-016 : une séance normale provient d'un planning publié.`) | Séance normale ⇐ planning publié | Toute séance `exceptional=false` porte une référence à une `planning_entry` d'une version `PUBLISHED` |
...
|---|---|---|---|---|---|---|---|
| EF-SES-004 | CDC §44 (`EF-SES-004 \| Annuler une séance \| SHOULD`) | Annuler une séance | `NOT_IMPLEMENTED` (CS : « pas de `PATCH` / annulation / remplaçant ») | `POST /api/v1/sessions/{id}/cancel` (motif obligatoire) : `PLANNED`/`OPEN` → `CANCELLED` ; `CANCELLED` non ouvrable ; aucun jeton ; aucune absence dérivée ; audit ; `409` métier | `CourseSessionService.cancel`, migration `V13` si nécessaire | tests transitions + concurrence + `403` + audit | Annuler une séance exceptionnelle |
| EF-SES-005 | CDC §44 (`EF-SES-005 \| Affecter un remplaçant \| SHOULD`) | Affecter un remplaçant | `NOT_IMPLEMENTED` (CS) | `POST /api/v1/sessions/{id}/substitute` : compte actif rôle `TEACHER`, formateur initial conservé, exception historisée, notification après commit (G1-D) | `teacher_substitution` (MDD §18.3), `SubstitutionService` | tests éligibilité + accès + audit | Désigner un remplaçant |
| RG-012 | CDC §43 (`RG-012 : un remplacement est autorisé et audité.`) | Remplacement autorisé + audité | Toute substitution écrit un `audit_event` catégorie `COURSE_SESSION` |
| RG-015 | CDC §43 (`RG-015 : une séance peut posséder un remplaçant autorisé.`) | Séance ⇒ remplaçant possible | `course_session.substitute_teacher_id` renseigné par la substitution |
| — | CDC §15.1 (« Une séance exceptionnelle peut être créée par un responsable pédagogique ») + CS (« pas de `PATCH` ») | Modifier une séance exceptionnelle `PLANNED` | `PATCH /api/v1/sessions/{id}` limité aux séances `exceptional=true` **et** `PLANNED` ; séance issue d'un planning non modifiable structurellement (DEC-G1-004) ; verrou optimiste → `409` | `CourseSessionService.update` | tests `PLANNED` vs `OPEN`/`CLOSED` + planning vs exceptionnel + concurrence |
...
| ID | Source | Résumé fidèle | Statut avant G1 | Critère d'acceptation G1 | Preuve code | Preuve test | Démo |
|---|---|---|---|---|---|---|---|
| EF-NOTIF-001 | CDC §44 (`EF-NOTIF-001 \| Afficher des notifications internes \| SHOULD`) | Afficher des notifications internes | `PARTIAL` (CS : « email d'activation seulement ») | Table `notification` (migration dédiée) ; `GET /api/v1/me/notifications` (paginé, borné, par utilisateur), `unread-count`, `read`, `read-all` ; front cloche + badge + liste | module `notification` étendu, migration `V14` | tests sécurité (isolation destinataire), pagination, contenu sans PII/jeton | Cloche → liste → marquer lu |
...
| ID | Source | Résumé fidèle | Statut avant G1 | Critère d'acceptation G1 | Preuve code | Preuve test | Démo |
|---|---|---|---|---|---|---|---|
| EF-JUS-001 | CDC §44 (`EF-JUS-001 \| Déposer un justificatif \| SHOULD`) | Déposer un justificatif | `PARTIAL` (CS : « justificatif **métier sans fichier** ») | Upload PDF/JPEG/PNG borné, contrôle extension + MIME + magic bytes, nom neutralisé, SHA-256, stockage **hors webroot** via port `JustificationFileStorage`, métadonnées en MySQL, aucun contenu en base | migration `V15`, entité `JustificationAttachment`, port de stockage + implémentation locale | tests PDF/JPEG/PNG OK, extension trompeuse rejetée, magic bytes, taille, traversal, en-têtes `Content-Disposition: attachment` + `nosniff` | Déposer un PDF fictif |
