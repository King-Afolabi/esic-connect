# Historique détaillé du projet — ESIC Connect (archive)

> **Ce fichier est une archive.** Il conserve, sans modification de fond,
> l'intégralité du contenu que portait `docs/CURRENT-STATE.md` jusqu'au
> checkpoint de finalisation **F2** (31 août 2026, base `main` =
> `e44ccb1`). Il est déplacé ici pour cesser de mélanger l'état courant
> et la chronologie détaillée des tranches PR #1 → PR #26 (FINAL-023).
>
> - **État courant, court et à jour** : `docs/CURRENT-STATE.md`.
> - **Audit de référence de fin de tranche** :
>   `docs/reports/PROJECT_FINAL_AUDIT.md` (checkpoint F1).
> - **Rapport de finalisation F2 → F6** :
>   `docs/reports/PROJECT_FINALIZATION_REPORT.md` (produit en fin de lot).
>
> Les affirmations ci-dessous (« branche non fusionnée », « aucune PR »,
> « prochaine priorité = émargement », chiffres de tests intermédiaires…)
> reflètent l'état **au moment où elles ont été écrites** et ne sont plus
> tenues à jour ici. Ne rien conclure de ce fichier sans recouper avec
> l'état courant et `git log`.

---

# État courant — ESIC Connect (contenu archivé au 31 août 2026)

> **⚠️ Audit de finalisation F1 (31 août 2026) — voir
> [`docs/reports/PROJECT_FINAL_AUDIT.md`](PROJECT_FINAL_AUDIT.md).**
...
# Chronologie du grand lot produit G1 (contenu archivé au 2 septembre 2026)

> **Archive.** Ces sections proviennent de `docs/CURRENT-STATE.md`, qui
> doit rester court. Elles décrivent bloc par bloc la livraison du grand
> lot produit **G1** (branche `feature/master-level-product-expansion`,
> fusionnée sur `main` par la **PR #40**, commit `d3450e6`), ainsi que
> les résultats d'audit des tranches F1 → F6 qui les précèdent.
>
> Elles sont conservées **verbatim** pour la traçabilité. L'état courant
> vérifié fait foi : `docs/CURRENT-STATE.md`. Le rapport de synthèse du
> lot est `docs/reports/G1_FINAL_REPORT.md` ; le journal détaillé bloc
> par bloc est `docs/reports/G1_IMPLEMENTATION_PROGRESS.md`.
...
> [`docs/reports/G1_IMPLEMENTATION_PROGRESS.md`](G1_IMPLEMENTATION_PROGRESS.md).
...
[`G1_IMPLEMENTATION_PROGRESS.md`](G1_IMPLEMENTATION_PROGRESS.md)
§ « Audit G1-C.3 ».
...
[`G1_IMPLEMENTATION_PROGRESS.md`](G1_IMPLEMENTATION_PROGRESS.md)
§ « Audit G1-D.1 » et
[`G1_REQUIREMENTS_TRACEABILITY.md`](G1_REQUIREMENTS_TRACEABILITY.md)
§5ter.
...
[`G1_FINAL_REPORT.md`](G1_FINAL_REPORT.md) et
...
[`docs/reports/G1_FINAL_REPORT.md`](G1_FINAL_REPORT.md).
...
/ `--amend`. Détail : [`docs/reports/G1_FINAL_REPORT.md`](G1_FINAL_REPORT.md)
§1bis / §3.4 / §7.
