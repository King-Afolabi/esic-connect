...
| TR-022 | Parcours d'émargement démontrable (séance exceptionnelle, cycle `PLANNED→OPEN→CLOSED`, jeton dynamique opaque + code court dans Redis, validation par un apprenant inscrit, anti-double présence par contrainte SQL, consultation des présences, interfaces formateur & apprenant, amorçage `demo`) | `coursesession`, `attendance`, `bootstrap`, `identity`, `academic`, `enrollment`, `audit`, `frontend` | `CourseSessionConstraintsTests`, `CourseSessionIntegrationTests`, `AttendanceRecordConstraintsTests`, `AttendanceTokenServiceTests`, `AttendanceIntegrationTests`, `AttendanceSecurityTests`, `DefaultDemoAccountProvisionerTests`, `DemoDataInitializerTests` ; front : `sessions-api.service`, `session-errors`, `qr-display`, `session-list`, `session-form`, `session-detail`, `attendance-check-in` | `./mvnw clean test` (V9 appliquée) + `npm test -- --watch=false` + `bash scripts/test/test-seed-demo.sh` + démonstration locale API (statuts HTTP relevés) + `scripts/seed-demo.sh` + `docs/11-guide-demonstration.md` — totaux courants dans `docs/CURRENT-STATE.md` | BC02/BC03 |
| TR-023 | Gestion de l'assiduité et reporting (V10, **fusionnée sur `main`** via PR #22 = `35bd04b`) : plusieurs points de contrôle par séance + jeton par point de contrôle, calcul des retards, présence manuelle / correction / annulation logique avec historique append-only, justificatif **métier sans fichier** (dépôt / examen, `ABSENT → EXCUSED_ABSENCE`), calcul de demi-journées (contexte d'alternance `COMPANY` exclu, `UNKNOWN` signalé), rapports séance / classe / apprenant + synthèse, **export CSV** avec neutralisation d'injection de formule, écrans `/sessions` enrichi + `/my-attendance` + `/attendance-management` | `coursesession`, `attendance`, `alternation` (port `AlternationDirectory`), `enrollment`, `academic`, `audit`, `frontend` | back : `AttendanceManagementConstraintsTests`, `CourseSessionConstraintsTests`, `CourseSessionIntegrationTests`, `AttendanceRecordConstraintsTests`, `AttendanceTokenServiceTests`, `AttendanceIntegrationTests`, `AttendanceSecurityTests`, `AttendanceReportSortTests` ; front : `sessions-api.service`, `attendance-api.service`, `my-attendance-list`, `attendance-management`, `session-detail` | `./mvnw clean test` + `npm test -- --watch=false` + `npm run lint` + `npm run build` + démonstration locale API (statuts HTTP, `docs/11` §10) — totaux courants dans `docs/CURRENT-STATE.md` et `docs/reports/PROJECT_FINAL_AUDIT.md` §4 | BC02/BC03 |
| TR-024 | Import CSV contrôlé des apprenants (V11, **fusionné sur `main`** via PR #23 `e8fd16d`, PR #24 `31acb09`, PR #25 `9c5affa`) : lecture sécurisée (RFC 4180 maison, UTF-8 strict, rejet ZIP/OLE2/PDF, séparateur auto, fichier jamais écrit — empreinte SHA-256 seule, `2 MiB` → `413`), **simulation** sans écriture métier (invariant T1), **confirmation** transactionnelle unique (verrou `FOR UPDATE`, re-validation, idempotence `APPLIED`, rollback total T3, e-mail `AFTER_COMMIT` T4), génération atomique du numéro `ESIC-{annéeDébut}-{NNNNN}`, audit `AFTER_COMMIT` + `REQUIRES_NEW` sans trace si rollback (T5), purge `@Scheduled`, 6 endpoints `/api/v1/student-imports` + décision fine de périmètre `PEDAGOGICAL_MANAGER`, front `/students/import` | `studentimport`, `identity` (port `StudentAccountProvisioner`), `enrollment` (port `StudentEnrollmentProvisioner`), `academic` (`ClassGroupDirectory.resolveForImport`), `audit` | back : `CsvFileGuardTests`, `CsvParserTests`, `CsvValueNormalizerTests`, `CsvRowNormalizerTests`, `StudentImportFieldValidatorTests`, `FileDuplicateDetectorTests`, `PlannedActionResolverTests`, `StudentImportSchemaConstraintsTests`, `StudentImportSimulationIntegrationTests`, `StudentImportConfirmationIntegrationTests`, `StudentImportConfirmationRollbackTests`, `StudentNumberAllocatorTests`, `StudentImportProvisionerContractTests`, `StudentImportAuditIntegrationTests`, `StudentImportApiIntegrationTests`, `StudentImportLifecycleApiIntegrationTests`, `StudentImportRecetteTests` ; front : `student-import-*.spec` | `./mvnw clean test` (V11) + `npm test -- --watch=false` (`student-import-*.spec`) — totaux courants dans `docs/CURRENT-STATE.md` ; couvre EF-IMP-001/002, US-050/051, RG-020..024, AC-004/005/006, TI-001..012 | BC02/BC03 |

> **Note F2 (31 août 2026)** : la section « Avancement vérifié » ci-dessous
> date du 28 août 2026 et cite des totaux de tests intermédiaires
> (499 / 416 / 502 / 548 / 454) qui **ne sont plus les chiffres
> courants**. Les totaux à jour (run F1, 31 août : back **682**, front
> **471**, `lint` OK, `build` **483,26 kB**, `npm audit` 0 vuln.) vivent
