...
| TR-023 | Gestion de l'assiduité et reporting (V10, branche `feature/attendance-management-and-reporting`, **non fusionnée**) : plusieurs points de contrôle par séance + jeton par point de contrôle, calcul des retards, présence manuelle / correction / annulation logique avec historique append-only, justificatif **métier sans fichier** (dépôt / examen, `ABSENT → EXCUSED_ABSENCE`), calcul de demi-journées (contexte d'alternance `COMPANY` exclu, `UNKNOWN` signalé), rapports séance / classe / apprenant + synthèse, **export CSV** avec neutralisation d'injection de formule, écrans `/sessions` enrichi + `/my-attendance` + `/attendance-management` | `coursesession`, `attendance`, `alternation` (port `AlternationDirectory`), `enrollment`, `academic`, `audit`, `frontend` | back : `AttendanceManagementConstraintsTests`(9), `CourseSessionConstraintsTests`(10), `CourseSessionIntegrationTests`(10), `AttendanceRecordConstraintsTests`(7), `AttendanceTokenServiceTests`(19), `AttendanceIntegrationTests`(25), `AttendanceSecurityTests`(8) ; front : `sessions-api.service`, `attendance-api.service`, `my-attendance-list`, `attendance-management`, `session-detail` | `./mvnw clean test` (V10 appliquée, **548** ; 532 puis 545 aux passes antérieures de revue PR #22) + `npm test -- --watch=false` (**454**) + `npm run lint` + `npm run build` (482 kB) + démonstration locale API (statuts HTTP, `docs/11` §10) | BC02/BC03 |
