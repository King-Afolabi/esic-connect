...
| **Statut** | `DECLARED` — pattern partagé spécifié (Phase 6). Application aux ~13 écrans : `DECLARED`, non livrée intégralement dans cette passe (large surface, chaque écran exige une vérification dialogues/mobile/impression/tables imbriquées). |
| **Tests de non-régression attendus** | Test de composant sur la variante partagée : au-delà du seuil de lignes, le conteneur a une `max-height` et `overflow-y:auto` ; `thead` reste visible au défilement (attribut `sticky` posé) ; en deçà du seuil, aucune hauteur imposée ; le `<body>` ne défile pas horizontalement. Vérification manuelle : impression et dialogues non affectés. |
...
| ANO-UX-002 | Moyenne | `DECLARED` (pattern spécifié) | Non — surface trop large pour une passe vérifiable |
