...
| **Statut** | Backend : `DECLARED` — voir Phase 3 (rapport avant correction). Correction runtime backend **non déployée** : elle exige une mesure `EXPLAIN ANALYZE` sur la Pi et un cycle build+backup+deploy que la présente passe ne peut pas conduire de façon vérifiable sans risque production (Pi hors d'atteinte cette session, porteur hors LAN). **Résilience front-end : `FIXED_LOCAL` (8 sept.)** — `Dashboard` : (1) garde de séquence (`dashSeq`) — une réponse arrivée après un rechargement plus récent (changement de contexte de rôle, « Réessayer ») est **ignorée**, elle ne peut plus écraser un état frais ni laisser un spinner tourner (équivalent `switchMap`) ; (2) l'échec passe par `normalizeHttpError` partagé et la bannière d'erreur **cite l'identifiant de corrélation** quand le corps le fournit (`.dashboard__error-ref`) ; (3) états `loading` / `forbidden` / `error` + « Réessayer » **préexistants** conservés. `dashboard.spec.ts` +2. **Note** : le `/me/dashboard` est un **appel unique** — la résilience « par carte » du résultat attendu supposerait de scinder l'endpoint en plusieurs routes ; non fait (aucune route inventée). Le vrai levier reste la correction backend. |
...
| **Statut** | Backend : `DECLARED` — diagnostic Phase 2 conduit sur le code ; `EXPLAIN ANALYZE` sur la Pi et correction runtime (borne de période + `GROUP BY` SQL + dénominateur) **non conduits** (même raison qu'ANO-PERF-001 ; Pi hors d'atteinte). **Résilience front-end : `IMPLEMENTED_AND_TESTED`** — constat 8 sept. : `attendance-summary` porte **déjà** garde de réponse obsolète (`loadToken`), états `loading` / `forbidden` / `error` par écran et bouton « Réessayer » ; `attendance-report` (coquille) porte déjà les mêmes états + `Réessayer`. Le seul écran qui manquait la garde anti-course était `Dashboard` — corrigé (voir ANO-PERF-001). **Pagination `Slice`** de rapport : `DECLARED` (backend). |
...
| **Statut** | `FIXED_LOCAL` (8 septembre 2026) — **comparaison et simulation en lecture seule livrées localement ; fusion réelle non implémentée ; déploiement différé.** Back-end : `POST /api/v1/users/duplicates/compare` (`ADMIN` / `SUPER_ADMIN`), lecture seule stricte — transaction `readOnly`, aucune écriture, aucun message d'outbox, aucune notification, aucune trace d'audit, `updated_at` intact. Volume des données rattachées remonté par un port entrant `DuplicateDependencyContributor` implémenté par `enrollment` / `attendance` / `claim` / `notification` (renverse la dépendance, `ModularityTests` vert — 19 modules). Front : sélection à cases, plafond à deux, compteur `0/2`–`2/2`, « Comparer » désactivé hors de deux, panneau `.esic-reveal` côte à côte (concordances / différences / conflits / avertissements / dépendances / verdict), fermeture + restauration du focus, bouton « Fusionner » **désactivé**. **Reliquat `DECLARED`** : affectations pédagogiques (`academic`) non encore remontées ; comparaison de plusieurs groupes ; fusion réelle (hors périmètre, décision porteur). Déploiement Pi + recette navigateur : `NOT_PERFORMED` (pas d'accès Pi cette session). |
| **Tests de non-régression attendus** | ✅ Back-end `DuplicateComparisonIntegrationTests` (15) : autorisé compare deux doublons ; rôle non autorisé → 403 ; non authentifié → 401 ; deux identifiants identiques → 400 `USER_COMPARE_SAME` ; UUID inconnu → 404 ; correspondances / différences / conflits calculés ; résumé des dépendances additionné ; `POTENTIALLY_SAFE` quand l'historique est concentré ; aucune entité modifiée ; aucun `outbox_message` / `audit_event` / `notification` créé ; aucun secret dans la réponse ; coût SQL borné et stable avec un historique volumineux. Front `duplicate-list.spec.ts` (+9) : sélection 0/2–2/2, plafond à deux, activation de « Comparer », POST des deux UUID, rendu côté à côte, états chargement / erreur + réessai, fermeture + focus, aucun bouton de fusion **actif**, accessibilité (libellés `aria`). |
...
| **Statut** | `FIXED_LOCAL` — le socle d'accessibilité des onglets **préexistait** et répond aux critères (trait visible, `aria-current`, focus, clavier, persistance par URL). Livré : (a) sous-navigation `.esic-subnav` de la page « Invitations » ; (b) **8 sept.** — convergence de `.academic__tabs` (liste + détail des référentiels académiques) vers la primitive `.esic-subnav` : `<nav class="esic-subnav"><div class="esic-subnav__list"><a class="esic-subnav__link" routerLinkActive="esic-subnav__link--active" ariaCurrentWhenActive="page">`, ~60 lignes de SCSS bespoke (`.academic__tabs*`, `.academic__tab--active`) supprimées dans les deux `.scss`. `academic-reference-detail.spec.ts` mis à jour (sélecteurs `nav.esic-subnav` / `.esic-subnav__link--active`). **Plus de reliquat.** Déploiement Pi impossible cette session (porteur hors LAN). |
...
| **Correction partielle livrée (8 septembre 2026)** | `PARTIAL`. **Livré** : (1) en-tête compact `.org__detail-header` — retour `.esic-back` (préexistant) + `Site {code}` + **pastille de statut** + actions **Modifier / Archiver / Restaurer** remontées à côté du titre (elles étaient dans une `.org__actions-row` plus bas) ; (2) les **trois sous-listes** (Bâtiments, Salles, Plages réseau) passent en `.esic-table-wrap--tall` + `matHeaderRowDef sticky` — chaque table devient une boîte à défilement borné : la page cesse d'être « trop longue » sans onglets ; (3) **filtre texte côté client** au-dessus de chaque sous-liste (`.org__list-filter`, `computed` `filtered{Buildings,Rooms,Ranges}`) : code / nom / étage / bâtiment pour les salles — répond à « gestion efficace de nombreuses salles » sans appel réseau (les sous-listes tiennent déjà en mémoire, `size=100`) ; (4) le panneau QR reste **en flux** sous la table des salles (table toujours visible — décision « aucune fenêtre modale » du dépôt). |
| **Statut** | `PARTIAL` / `FIXED_LOCAL` (déploiement Pi impossible cette session — porteur hors LAN). Tests : `site-detail.spec.ts` +2 (enveloppes `--tall` sur ≥ 2 tables + entête `sticky` ; filtre salles local, aucun appel, message « aucun résultat »). Suite frontend **862 tests / 0 échec** ; `ng build` prod **582,62 kB**, aucune alerte de budget. |
| **Reste `DECLARED`** | Découpage **en onglets** proprement dit (une seule section visible à la fois, `role="tablist"` bespoke sur la primitive `.esic-subnav` — le dépôt n'utilise **pas** `mat-tab-group`, cf. `docs/03-architecture.md` §système de design ; à faire avec revue visuelle clavier / point de rupture, pas à l'aveugle) + persistance de l'onglet dans l'URL (`?tab=`). **Pagination serveur par onglet** : non faite — les endpoints `buildings` / `rooms` / `network-ranges` sont chargés à `size=100` sans pagination ; poser un `mat-paginator` sur un fetch non paginé serait cosmétique (aucune route inventée). |
| **Tests de non-régression attendus (reliquat onglets)** | Test de composant : la fiche rend 4 onglets, un seul actif, `aria-selected` ; l'onglet Plages réseau n'apparaît que pour `SUPER_ADMIN` ; l'onglet actif est restauré après navigation ; « Afficher QR » présent sur chaque ligne de salle (déjà le cas) ; retour vers `/organization/sites` en tête (déjà le cas). |
...
|---|---|---|---|
| ANO-PERF-001 | Majeure | Backend `DECLARED` (diagnostic + spec) ; front `FIXED_LOCAL` — `Dashboard` : garde anti-course + `normalizeHttpError` + réf. de corrélation dans l'erreur | Backend : non (hors passe, Pi hors LAN) ; front : oui |
| ANO-PERF-002 | Majeure | Backend `DECLARED` (borne période + `GROUP BY` + `Slice`) ; front : résilience **déjà en place** (`attendance-summary` / `attendance-report` : garde `loadToken`, états par écran, Réessayer) | Backend : non ; front : rien à livrer |
| ANO-USER-001 | Moyenne | `FIXED_LOCAL` (8 sept. — comparaison + simulation lecture seule ; fusion réelle non implémentée ; déploiement différé) | Non — Pi injoignable cette session |
| ANO-UX-001 | Moyenne | `FIXED_LOCAL` — socle préexistant conforme ; sous-nav Invitations + convergence `.academic__tabs` → `.esic-subnav` livrées ; plus de reliquat | Oui (frontend) |
| ANO-UX-002 | Moyenne | `FIXED_LOCAL` — primitive `.esic-table-wrap--tall` + 10 écrans (attendance-report inclus) ; déploiement Pi en attente (porteur hors LAN) | Oui (frontend) — plus de reliquat |
| ANO-UX-003 | Moyenne | `PARTIAL` / `FIXED_LOCAL` — en-tête compact + statut + actions remontées ; 3 sous-tables `--tall`/sticky ; filtre texte par sous-liste. Reste : découpage en onglets bespoke + `?tab=` (revue visuelle requise) | Oui (frontend) pour la part livrée ; onglets = passe dédiée |
