...
| R-G1-15 | Volume disque non borné (pièces jointes) | 3 | 3 | 9 | Taille bornée (`JUSTIFICATION_MAX_FILE_BYTES`) ; dette de purge documentée (`docs/08-securite-rgpd.md`) ; suivi (DEC-G1-008) |
...
| R-G1-21 | Absence de test e2e navigateur | 3 | 3 | 9 | **Fermé le 03/09/2026.** État au 31/08 : assumé (`DEC-G1-011`), Playwright non retenu pour coût disproportionné, aucune tentative d'installation. La suite a finalement été construite pendant l'audit QA : 149 tests Playwright / Chromium (`tests/`). `DEC-G1-011` est **révisée**, pas contournée : la suite est conservée en **complément** de la recette d'intégration API, jamais en remplacement — la recette API ne pilote aucun navigateur et ne rend aucun composant Angular. Risque résiduel suivi en **R-QA-06** (§7ter) : coût d'exploitation de la suite et instabilité d'environnement observée |
...
| R-G1-30 | Table `notification` sans borne de croissance ni politique de conservation | 3 | 2 | 6 | **Assumé (audit G1-D.1)** : aucune durée documentaire (MDD §23.1 ne fixe rien) ; **aucune purge inventée** ; `docs/08` §14 = rétention notifications `À_DÉFINIR` ; conformité RGPD non revendiquée sur ce point ; dette **G1-D-RETENTION** (`docs/05` §9bis) |
...
| R-G1-36 | Absence de rate-limiting sur `/auth/login` | 3 | 4 | 12 | **Ouvert — dette assumée (`docs/08` §5)** : un limiteur *fail-safe*, sans énumération de comptes et testé, dépasse le périmètre livré. Atténuations en place : refus **uniforme** quel que soit le motif d'échec, hachage BCrypt, aucun message distinguant email inconnu / mot de passe faux / compte inactif |
...
En cas de panne pendant une démonstration :
