...
`student_import_job` / `_job_issue` / `_row` / `_row_issue`) ;
téléversement multipart `.csv` (fichier **non conservé** : nom assaini +
SHA-256) ; séparateur `,`/`;` auto-détecté, UTF-8 ; ≤ 2 MiB et ≤ 500
lignes ; simulation persistant **uniquement** des lignes techniques
temporaires (RG-020) ; confirmation en **une** transaction avec verrou
pessimiste, re-validation complète, idempotence et `409
IMP_STALE_SIMULATION` si la simulation n'est plus à jour (aucune
application partielle) ; nouveaux ports `identity.StudentAccountProvisioner`
(création compte `PENDING_ACTIVATION` + rôle `STUDENT` + invitation,
réutilise `AccountInvitationService`) et `enrollment.StudentEnrollmentProvisioner`,
extension `academic.ClassGroupDirectory.resolveForImport(codes)` ;
endpoints `POST /api/v1/student-imports`, `GET .../{id}`, `GET .../{id}/rows`,
...
`P7D` vs 30–90 jours docs/07, non-conservation du fichier, tables
dédiées vs `import_job` générique docs/04 §16, `student_number`
obligatoire à la création, application non partielle, pas de réécriture
d'identité, confirmation par un tiers, config multipart à ajouter).
Découpage en 11 checkpoints (CP0 = ce rapport).
...
| Import apprenants | CONÇU (Checkpoint 0 — `docs/reports/STUDENT_CSV_IMPORT_DESIGN.md` ; branche `feature/student-csv-import`, PR ouverte non fusionnée ; module `studentimport` + migration `V11` prévus ; aucun code produit) |
